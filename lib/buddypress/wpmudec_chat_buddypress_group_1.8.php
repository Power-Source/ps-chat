...
class PSOURCE_Chat_BuddyPress extends BP_Group_Extension {
	...
	const settings_slug = 'psource_chat_bp_group';
	...
	function settings_screen( $group_id = null ) {
		...
		if ( $bp_group_id ) {
			...
			if ( ( groups_is_user_mod( $bp->loggedin_user->id, $bp_group_id ) )
				...
				// Use WordPress Color Picker instead of deprecated Farbtastic
				?>
				<link rel='stylesheet' id='wp-color-picker-css'
				      href='<?php echo includes_url(); ?>/css/wp-color-picker.min.css'
				      type='text/css' media='all'/>
				<script type='text/javascript' src='<?php echo includes_url(); ?>/js/wp-color-picker.min.js'></script>
				<?php
	...
	function settings_screen_save( $group_id = null ) {
		...
		if ( $bp_group_id ) {
			...
			if ( ( groups_is_user_mod( $bp->loggedin_user->id, $bp_group_id ) )
				...
				$chat_settings['id']           = 'psource-chat-bp-group-' . $bp_group_id;
				...
				if ( ! is_admin() ) {
					...
					if ( ! $success ) {
						bp_core_add_message( __( 'Fehler beim speichern. Bitte versuche es erneut', 'buddypress' ), 'error' );
					} else {
						bp_core_add_message( __( 'Einstellungen erfolgreich gespeichert', 'buddypress' ) );
					}
				}
			} else {
