 ...
 * @global    int $blog_id
 */
function psource_chat_get_last_chat_id() {
	//global $wpdb, $blog_id;
	//$last_id = $wpdb->get_var("SELECT chat_id FROM `".PSOURCE_Chat::tablename('message')."` WHERE blog_id = '{$blog_id}' ORDER BY chat_id DESC LIMIT 1");
...
function psource_chat_get_wp_roles() {
	global $wp_roles;
 ...
 */
function psource_chat_check_is_blocked_urls( $urls = array(), $blocked_urls_action = 'exclude', $DEBUG = false ) {
	...
	if ( ( is_array( $urls ) ) && ( count( $urls ) ) ) {
		...
		$blocked_urls = apply_filters( 'psource-chat-blocked-site-urls', $blocked_urls );
		if ( ! empty( $blocked_urls ) ) {

			//$blocked_urls_action = $psource_chat->get_option('blocked_urls_action', $section);
...
function psource_chat_start_performance( $chat_performance = array() ) {
...
function psource_chat_end_performance( $chat_performance = array() ) {
	...
	$chat_performance['memory_summary']        = psource_chat_utility_size_format( $chat_performance['memory_end'] - $chat_performance['memory_start'], 4 );
	$chat_performance['memory_limit']          = psource_chat_utility_size_format( $chat_performance['memory_limit'], 4 );
	$chat_performance['memory_start']          = psource_chat_utility_size_format( $chat_performance['memory_start'], 4 );
	$chat_performance['memory_end']            = psource_chat_utility_size_format( $chat_performance['memory_end'], 4 );
	$chat_performance['memory_get_peak_usage'] = psource_chat_utility_size_format( $chat_performance['memory_get_peak_usage'], 4 );
...
function psource_chat_utility_size_format( $bytes = 0, $precision = 2 ) {
	$kilobyte = 1000;
...
function psource_chat_get_active_sessions_users( $chat_sessions = array() ) {
	global $wpdb;

	$chat_sessions_users = array();

	foreach ( $chat_sessions as $chat_session ) {
		$sql_str = $wpdb->prepare( "SELECT count(*) FROM " . PSOURCE_Chat::tablename( 'users' ) . " WHERE chat_id=%s AND blog_id=%d", $chat_session['id'], $chat_session['blog_id'] );
