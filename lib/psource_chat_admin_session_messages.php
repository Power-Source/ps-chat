...
if ( ! class_exists( 'PSOURCEChat_Session_Messages_Table' ) ) {
	class PSOURCEChat_Session_Messages_Table extends WP_List_Table {
		...
		function get_table_classes() {
			return array( 'widefat', 'fixed', 'psource-chat-session-messages-table' );
		}
		...
		function check_table_filters() {
			...
			$sql_str = $wpdb->prepare( "SELECT * FROM " . PSOURCE_Chat::tablename( 'log' ) . " WHERE id=%d", $this->filters['chat-log-id'] );
			if ( is_multisite() ) {
			...
			if ( $this->moderator_names_loaded == false ) {
				if ( $this->log_item->archived == 'no' ) {
					$sql_str = $wpdb->prepare( "SELECT name FROM " . PSOURCE_Chat::tablename( 'message' ) . " WHERE moderator = %s AND chat_id = %s AND archived = %s GROUP BY name ORDER BY name", 'yes', $this->filters['chat_id'], 'no' );

				} else {
					$sql_str = $wpdb->prepare( "SELECT name FROM " . PSOURCE_Chat::tablename( 'message' ) . " WHERE moderator = %s AND chat_id = %s AND log_id = %d GROUP BY name ORDER BY name", 'yes', $this->filters['chat_id'], $this->filters['chat-log-id'] );
				}
			...
			if ( $this->user_names_loaded == false ) {
				if ( $this->log_item->archived == 'no' ) {
					$sql_str = $wpdb->prepare( "SELECT name FROM " . PSOURCE_Chat::tablename( 'message' ) . " WHERE moderator = %s AND chat_id = %s AND archived = %s GROUP BY name ORDER BY name", 'no', $this->filters['chat_id'], 'no' );
				} else {
					$sql_str = $wpdb->prepare( "SELECT name FROM " . PSOURCE_Chat::tablename( 'message' ) . " WHERE moderator = %s AND chat_id = %s AND log_id = %d GROUP BY name ORDER BY name", 'no', $this->filters['chat_id'], $this->filters['chat-log-id'] );
